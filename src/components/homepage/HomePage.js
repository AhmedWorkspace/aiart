...
const HomePage = () => {
  ...
  const col2_images = [];
  const col3_images = [];
  const sollpic = useRef(null);

  const tooltiphandleshow = () => {
    setshowtooltip(true)
  };

  const tooltiphandlehide = () => {
  ...
  const onscrollgo = (e)=>{
    ...
    if (e.key === 'Enter') {
      ...
      sollpic.current?.scrollIntoView({behavior: 'smooth'});
    }
}
      ...
      <section className={styles.main_section}>
        <div className={styles.cover_img}>
          <div className={styles.text_onImg}>
            ...
            <h2>
              Find the perfect image for your project, fast. With our search
              engine and ai-powered <br /> image generation, you'll be able to
              get the visuals you need, quickly and easily.
            </h2>
            <form onSubmit={onFormSubmit}>
              ...
              <input
                ...
                name=""
                id=""
                placeholder="Search free image prompts"
                onMouseOver={tooltiphandleshow}
                onMouseLeave={tooltiphandlehide}
                onKeyUp={onscrollgo}
              />
            ...
            <p>
              Trending:&nbsp; <Link to={"/category/Abstract"}>Abstract</Link>,
              &nbsp; <Link to={"/category/Sports"}> Sports </Link>,&nbsp;
              <Link to={"/category/Space"}> Space </Link>,&nbsp;
              <Link to={"/category/Holidays"}> Holidays </Link>,{" "}
              <Link to={"/category/Animals & Wildlife"}>
                Animals & Wildlife
              </Link>
              {showtooltip && <Tooltip />}
            </p>
            <p className={styles.read_more}>
              Read more about the <Link to={"./license"}>Ai-Arts License</Link>
            </p>
          </div>
        </div>

        <div className={styles.discription} id="allimagess" ref={sollpic}>
          <div className={styles.col}>
            <h4>With our easy to use search prompts,</h4>
            <p>you can easily find images that are sure to impress.</p>
          </div>
          <div className={styles.col}>
            <h4>You can create any picture you want</h4>
            <p>
              You don't have to search anymore. Our AI can create any photo you
              want.
            </p>
          </div>
          <div className={styles.col}>
